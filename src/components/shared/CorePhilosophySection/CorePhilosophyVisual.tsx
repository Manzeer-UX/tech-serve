...
export function CorePhilosophyVisual({
  ...
  return (
    <RevealInView delay={0.08} className="relative mx-auto w-full max-w-[42rem]">
      <div className="relative overflow-hidden rounded-[2.75rem] border border-[var(--color-border-strong)] bg-[linear-gradient(180deg,rgba(251,247,255,0.97),rgba(243,236,255,0.92))] px-5 py-5 shadow-[var(--shadow-hero)] md:px-7 md:py-7">
        ...
        <div className="absolute left-[20%] top-[18%] h-40 w-40 rounded-full bg-[var(--color-glow-secondary)] blur-3xl opacity-50" />
        <div className="absolute bottom-[12%] right-[18%] h-44 w-44 rounded-full bg-[var(--color-glow-primary)] blur-3xl opacity-45" />

        <div className="relative min-h-[28rem]">
          <div className="absolute left-0 top-0 rounded-[1.1rem] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] px-4 py-3 shadow-[var(--shadow-pill)] backdrop-blur-sm">
          ...
          <div className="absolute right-0 top-8 w-full max-w-[16.5rem] rounded-[1.7rem] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] px-4 py-4 shadow-[var(--shadow-card)] backdrop-blur-sm">
              ...
              <div className="flex items-end gap-2">
                {chartBarHeights.map((height, index) => (
                  <div key={chartBarLabels[index]} className="flex flex-col items-center gap-2">
                    <span
                      className={`w-3.5 rounded-full bg-[var(--color-brand-black)] ${height}`}
          ...
          <div className="absolute bottom-14 left-0 w-full max-w-[15rem] rounded-[1.6rem] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] px-4 py-4 shadow-[var(--shadow-card)] backdrop-blur-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[0.7rem] font-semibold uppercase tracking-[0.16em] text-[var(--color-secondary-purple)]">
                  {panel.scoreDetail}
                </p>
                <p className="mt-2 text-[1.7rem] font-semibold tracking-[-0.05em] text-[var(--color-brand-black)]">
                  {panel.scoreValue}
                </p>
                <p className="text-[0.84rem] text-[var(--color-muted)]">
                  {panel.scoreLabel}
            ...
            <div className="relative mt-4 h-20 rounded-[1.1rem] bg-[linear-gradient(180deg,rgba(246,240,255,0.92),rgba(255,255,255,0.74))]">
          ...
          <div className="absolute bottom-0 right-4 w-full max-w-[17rem] rounded-[1.4rem] border border-[var(--color-border-subtle)] bg-[var(--color-surface-card)] px-4 py-4 shadow-[var(--shadow-pill)] backdrop-blur-sm">
            <div className="flex items-center gap-4">
              <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full border border-[var(--color-border-strong)] bg-white text-[1rem] font-semibold text-[var(--color-brand-black)]">
                {panel.lineValue}
              </span>
