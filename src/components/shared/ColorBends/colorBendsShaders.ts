...
export const COLOR_BENDS_FRAGMENT_SHADER = `
...
void main() {
  ...
  if (uColorCount > 0) {
    ...
    for (int i = 0; i < MAX_COLORS; ++i) {
      ...
      float clampStrength = clamp(uWarpStrength, 0.0, 1.0);
      float mixStrength = pow(clampStrength, 0.3);
      float gain = 1.0 + max(uWarpStrength - 1.0, 0.0);
      vec2 warped = s + (r - s) * clampStrength * gain;
      float m1 = length(warped + sin(5.0 * warped.y * uFrequency - 3.0 * t + float(i)) / 4.0);
      float m = mix(m0, m1, mixStrength);
...
export const COLOR_BENDS_VERTEX_SHADER = `
varying vec2 vUv;
